    ...
    public void enviarConfirmacion(String to, String nombre, String productos, double total) {
        new Thread(() -> {
            try {
                ...
                String encodedUser = Base64.getEncoder().encodeToString(USERNAME.getBytes());
                ...
                String encodedSubject = Base64.getEncoder().encodeToString("=?UTF-8?B?" +
                        Base64.getEncoder().encodeToString("✅ Pedido Confirmado - GlobalLive".getBytes("UTF-8")) + "?=".getBytes());
                ...
                sendCommand(out, "Subject: =?UTF-8?B?" + Base64.getEncoder().encodeToString("✅ Pedido Confirmado - GlobalLive".getBytes("UTF-8")) + "?=");
